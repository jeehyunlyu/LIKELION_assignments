...
def bookmark(request):
    if request.method == 'POST':
        ...
        todo_bookmarks = Bookmark.objects.filter(
            todo = ToDo.objects.get(pk=todo_pk)
        )

        response = {
            'bookmark_count': todo_bookmarks.count(),
        }
...
def mypage(request):
    ...
    return render(request, 'mypage.html', {'todos': todos, 'comment': comment, 'like': like, 'bookmark': bookmark})
